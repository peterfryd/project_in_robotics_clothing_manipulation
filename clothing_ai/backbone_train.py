...
def masked_l1_loss(preds, targets):
    vis_mask = (targets[:, :, 2] > 0).unsqueeze(-1).float()  # only visible points
...
def mean_pixel_error(preds, targets, img_size):
    ...
    err = diff.sum(dim=-1) / 2.0  # average of x,y errors
    mean_err = err.sum() / vis_mask.sum().clamp(min=1)
    return mean_err.item() * img_size  # convert normalized → pixel scale
...
for epoch in range(EPOCHS):
    ...
    for imgs, landmarks in tqdm(train_loader, desc=f"Epoch {epoch+1}/{EPOCHS}"):
        imgs = imgs.to(device)
    ...
    print(f"📝 Epoch {epoch+1} | Val L1 Loss: {avg_val_loss:.4f} | Mean Pixel Error: {avg_val_pixel_err:.2f}px")
